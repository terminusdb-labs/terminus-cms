...
import { TERMINUS_CMS_KEY, TERMINUS_CMS_USER } from "../constants";
...
// to be review
        ...
        if(!name || name === "" || !password || password === "") {
            ...
            client.info().then(response=>{
                if(response["api:info"] && response["api:info"]['authority'] === "anonymous"){
                    ...
                    localStorage.setItem(TERMINUS_CMS_USER,name) 
    ...
    return <Modal size="lg" className="modal-dialog-right" show={showModal}  {...onHide}>
        <Modal.Header>
            <Modal.Title className="h6">Login to Terminus CMS</Modal.Title>
