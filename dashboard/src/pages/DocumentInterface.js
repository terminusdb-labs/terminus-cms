...
export const DocumentInteface = () => {
    ...
    const { client, frames, updateBranch } = ClientObj()
   ...
   return <main className="content mt-5 w-100">
        {showModal && <CreateChangeRequestModal showModal={showModal} setShowModal={setShowModal} updateViewMode={updateViewMode}/>}
        <Card className="ml-5 mr-5">
            <Card.Header className="justify-content-between d-flex w-100 text-break">
                ...
                {viewMode === "View" && <Button variant="light" type="button" title="Form View" onClick={(e) => startEditMode()} className="btn-sm btn d-flex text-dark">
                    Edit
                </Button>}
            </Card.Header>
